...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .codex/tools/aidlc-orchestrate.ts next --stage contract-design --single
   ```
...
3. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .codex/tools/aidlc-orchestrate.ts report --single --stage contract-design --result completed
   ```
